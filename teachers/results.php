<?php
// Teacher Results Management - Aggregate Grading Calculations
...
try {
    $stmt = $pdo->prepare("SELECT t.*, s.school_name, s.sms_enabled FROM teachers t JOIN schools s ON t.school_id = s.id WHERE t.id = ?");
...
<head>
    ...
    <title>Results - <?php echo htmlspecialchars($teacher_name); ?></title>
...
<body>
    ...
    <script>
        ...
        const smsEnabled = <?php echo json_encode($teacher['sms_enabled'] ?? 0); ?>;
        const schoolId = <?php echo json_encode($school_id); ?>;
        const schoolName = <?php echo json_encode($teacher['school_name'] ?? ''); ?>;
