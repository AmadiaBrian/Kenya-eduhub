<?php
// Receipt Generation API for Parents
...
try {
    ...
    error_log("Receipt generation request: parent_id=$parent_id, payment_id=$payment_id");
    
    // Get payment details and verify it belongs to this parent's child
    $stmt = $pdo->prepare("
        ...
        WHERE fp.id = ? AND sp.parent_id = ? AND fp.status = 'completed'
    ");
    $stmt->execute([$payment_id, $parent_id]);
    $payment = $stmt->fetch();
    
    if (!$payment) {
        error_log("Payment not found for parent_id=$parent_id, payment_id=$payment_id");
    ...
    echo json_encode([
        'success' => true,
        'receipt_url' => '/parents/receipts/' . $receipt_filename,
