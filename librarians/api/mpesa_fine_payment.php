<?php
// M-Pesa Fine Payment API for Librarians
...
// Debug session
error_log("Session data in mpesa_fine_payment: " . print_r($_SESSION, true));
error_log("Session ID: " . session_id());

// Check if librarian is logged in
if (!isset($_SESSION['librarian_id'])) {
    echo json_encode(['success' => false, 'error' => 'Unauthorized - No librarian session found']);
    exit;
}
...
try {
    // Debug input
    error_log("MPESA fine payment input: " . print_r($_POST, true));
    ...
    error_log("Fine query result: " . print_r($fine, true));
    
    if (!$fine) {
        echo json_encode(['success' => false, 'error' => 'Fine not found with ID: ' . $fine_id]);
        exit;
    }
    ...
    $stmt = $pdo->prepare("INSERT INTO book_history (book_id, school_id, action, user_id, user_type, details) VALUES (?, ?, 'fine_payment_initiated', ?, 'librarian', ?)");
    $stmt->execute([$fine['book_id'], $fine['school_id'], $librarian_id, "MPESA payment initiated: $amount for fine ID: $fine_id"]);
    
    // Include centralized accessToken.php for M-Pesa access token
    include __DIR__ . '/../../parents/api/accessToken.php';
    ...
    $callbackurl = MPESA_CALLBACK_BASE_URL . MPESA_LIBRARIANS_CALLBACK_PATH;
