<?php
/**
 * School B2C Withdrawal Processor
...
{
    $stmt = $pdo->prepare("INSERT INTO school_withdrawals (school_id, amount, destination_type, destination_name, destination_account, destination_extra, notes, status, reference_number, result_desc) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)");
    $stmt->execute([
...
try {
    ...
    // Rate limiting: Max 5 withdrawal requests per 5 minutes per school
    ...
    error_log('School B2C request: ' . $dataString);
    error_log("School B2C response HTTP $b2cStatus: " . $b2cResponse);
    error_log("School B2C phone: " . $phone);
    ...
    if ($b2cStatus !== 200 || ($responseData['ResponseCode'] ?? '') !== '0') {
        ...
        if (strpos($message, 'Initiator') !== false || strpos($message, 'initiator') !== false) {
            $message = 'The M-Pesa initiator credentials are invalid. Please check the Initiator Name and Security Credential in the configuration file (schools/config/mpesa_config.php). These must be obtained from the Safaricom Developer Portal.';
        }
    ...
    error_log('School B2C withdrawal failed: ' . $e->getMessage());
